<div class="container-fluid">
    <div class="row">
      <div class="col-2">
        <p>Copyright &copy; 2017 shopnepal</p>
        <span> All Rights Reserved</span>
      </div>
      <!--end of  col-2-->

      <div class="col-lg-6">
        <h2>About</h2>
        <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d220.93601079845456!2d85.3264455!3d27.6252546!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x39eb1710f57d4a77%3A0xae934ae08d66ec9c!2sDhapakhel+VDC+Health+Post!5e0!3m2!1sen!2snp!4v1510478607023" width="200" height="100" frameborder="5" style="border:5px" allowfullscreen  class=" pull-right thumbnail "> </iframe>
        <p>Created by: Shankar Ghimire  Lorem ipsum dolor sit amet, consectetur adipisicing elit. Officiis reprehenderit cupiditate deleniti ratione, a facere enim. Maiores amet voluptatibus pariatur magni doloribus non eligendi consectetur nam excepturi vel, nostrum laudantium!</p>
      </div>
      <!-- end of col-6-->
      <div class="col-sm-4">
        <p>Coded with <span class="fa fa-heart"></span> Shankar Ghimire</p>
      </div>
      <!--end of col-lg-4-->
    </div>
    <!--end of row-->
  </div>
  <!--end of container-->